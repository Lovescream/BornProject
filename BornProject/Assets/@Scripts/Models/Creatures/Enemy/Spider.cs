...
public class Spider : Enemy {
    ...
    protected static readonly int AnimatorParameterHash_Spider = Animator.StringToHash("Spider");
    ...
    private void OnEnteredChase() {
        _animator.SetBool(AnimatorParameterHash_Spider, true);
    }
    private void OnEnteredAttack() {
        Velocity = Vector2.zero;
    }
    private void OnEnteredDead() {
        _animator.SetBool(AnimatorParameterHash_Spider, false);
    }
    private void OnExitedChase() {
        if (Target == null) _animator.SetBool(AnimatorParameterHash_Spider, false);
    }
